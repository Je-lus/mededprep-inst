...
router.put('/:id', validate(updateSessionSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (startDateTime !== undefined) data.startDateTime = startDateTime ? new Date(startDateTime) : null;
...
router.post('/:id/attendees/:aid/check-in', async (req: Request, res: Response, next: NextFunction) => {
  try {
    await findSessionOrThrow(req.params.id, req.orgId!);
...
router.post('/:id/attendees/:aid/check-out', async (req: Request, res: Response, next: NextFunction) => {
  try {
    await findSessionOrThrow(req.params.id, req.orgId!);
