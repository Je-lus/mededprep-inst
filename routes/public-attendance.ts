...
router.post('/attend/:hash/register', validate(registerSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const session = await prisma.session.findFirst({
      where: {
        publicHash: req.params.hash,
...
router.post('/attend/:hash/lookup', validate(lookupSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const session = await prisma.session.findFirst({
      where: {
        publicHash: req.params.hash,
...
router.post('/attend/:hash/checkout', validate(checkoutSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const session = await prisma.session.findFirst({
      where: {
        publicHash: req.params.hash,
